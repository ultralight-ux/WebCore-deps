...
SET BUILD_DIR=build-win-msvc-2019-uwp
...
IF NOT EXIST "%ICU_DIR%\lib64uwp\icuuc.lib" CALL :EXIT_NO_ICU
IF NOT EXIST "%ICU_DIR%\lib64uwp\icuin.lib" CALL :EXIT_NO_ICU

echo Copying ICU build files to out directory.
XCOPY /Q /Y "%ICU_DIR%\lib64uwp\icuuc.lib" "%CMAKE_INSTALL_PREFIX%\lib\"
XCOPY /Q /Y "%ICU_DIR%\lib64uwp\icuin.lib" "%CMAKE_INSTALL_PREFIX%\lib\"
...
set MSVC_FLAGS="/Zc:inline /DUWP_PLATFORM=1 /FI %PLATFORM_DEFINES_HEADER%"
SET "CMAKE_FLAGS=-DCMAKE_BUILD_TYPE=%CMAKE_BUILD_TYPE% -DCMAKE_INSTALL_PREFIX=%CMAKE_INSTALL_PREFIX% -DCMAKE_SYSTEM_NAME=WindowsStore -DCMAKE_SYSTEM_VERSION=10.0 -DCMAKE_CXX_FLAGS=%MSVC_FLAGS% -DCMAKE_C_FLAGS=%MSVC_FLAGS%"
...
:EXIT_NO_ICU
  ...
  echo   'common_uwp' and 'i18n_uwp' projects.
  echo.
